...
async def test_construct_manifests_of_testpage_folder(get_debug_postage, bee_class):
    ...
    text_reference = upload_data(bytes([104, 97, 108, 105]), get_debug_postage, bee_class)


    i_node = MantarayNode()
    i_node.add_fork(b'index.html', hex_to_bytes(index_reference), {
        'Content-Type':'text/html; charset=utf-8',
        'Filename':'index.html',
    })
    i_node.add_fork(b'img/icon.png.txt', hex_to_bytes(text_reference), {
        'Content-Type':'',
        'Filename':'icon.png.txt',
    })
    i_node.add_fork(b'img/icon.png', hex_to_bytes(image_reference), {
        'Content-Type':'image/png',
        'Filename':'icon.png',
    })
    i_node.add_fork(b'/', bytes(32), {
